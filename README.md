...
SVM, RF, 그리고 CNN 모델을 데이터셋에 훈련시켰습니다. SVM_ECOC 모델은 0.7566의 정확도를, SVM_POLY 모델은 0.80041의 정확도를 보여주었으며, SVM_RBF 모델은 0.80382의 정확도를 기록했습니다. 이 중에서 SVM_RBF 모델이 가장 높은 정확도를 달성했고, RF 모델은 SVM 모델들보다 더 높은 0.85542의 정확도를 보여주었습니다. 또한, CNN 모델은 다른 모델들보다 우수한 성능을 보여주어 높은 수준인 0.9292의 정확도를 기록했습니다.

이 연구의 개선 방향으로는 먼저, 모델이 새로운 데이터셋을 학습할 수 있도록 하고, 얼굴 클래스 레이블을 분류하는 대신에 절대적인 기준을 정립해야 합니다. 또한, 하회탈 레이블이 적절한지 판단하기 위해 객관적인 검토가 필요합니다.

마지막으로, 이 연구의 활용 방향에 대해 언급하고자 합니다. 얼굴 사진을 통해 유사한 하회탈을 추정하는 모델은 인공지능을 우리의 문화와도 접목할 수 있다는 사실을 널리 알리는 계기를 만들 수 있을 것입니다.. 또한 ‘닮은 하회탈을 알아봄으로써 하회탈 문화를 체험하는 것’과 같이, 새로운 형태의 문화 체험 프로그램을 제공할 수 있을 것으로 기대합니다. 
