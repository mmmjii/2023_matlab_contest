...
데이터 출처 : https://www.kaggle.com/datasets/atulanandjha/lfwpeople
 ...
 원본 데이터 수는 총 1,323개이며, 정면을 보는 사진들만을 사용하였습니다. 그 이유로는 첫째, 본래 하회탈은 정면 부분만 존재하여 측면 등을 고려할 필요가 없다고 생각하였습니다. 두 번째로는 정면이 아닌 다른 곳을 바라보는 사진들은 이목구비에 대한 수치가 측정되지 않는 경우가 존재하여, 닮은 하회탈을 추정하는 데에 어려움이 있을 것이라 판단하였기 때문에 정면을 보는 사진만을 사용하였습니다. 정면을 바라보는 데이터를 걸러내는 기준으로는 얼굴의 특징점을 구한 뒤 좌/우 눈의 중앙에 위치한 점과 코의 중앙에 위치한 점을 이어 만들어진 각도를 구하여, 그 각이 90도에 가까우면 정면이라 판단하여 정면 사진을 걸러냈습니다. 추가로 눈, 코, 입, 턱이 가려져 있는 이미지들도 걸러냈습니다.
...
+ 4.1 상대적인 라벨링 문제
  ...
  이 프로젝트의 주제인 하회탈 추정과 관련하여 상대적인 라벨링이 필요했습니다. 하회탈은 사람과 비교했을 때 어떤 부분이 닮았는지 공식적인 기준이 없어 개인적인 주관으로만 판단할 수 있기 때문입니다. 이러한 이유로, 우리는 다양성을 확보하기 위해 최대한의 데이터 셋을 모으는 데 노력하였습니다. 원본 데이터 셋 1,323장을 적절히 가공하여 14,553장의 샘플을 얻었습니다. 이렇게 확장된 데이터 셋의 크기에 대한 적절성은 모델 성능을 통해 확인하였습니다. CNN 모델의 성능은 약 0.9로 나타났습니다. 하회탈의 탈 닮음 기준이 사물 분류의 기준과는 다소 모호함을 감안하면, 이 수치는 수용할 만한 결과라고 판단됩니다.


+ 4.2 RF와 SVM 모델의 성능이 CNN 모델에 비해 낮은 성능 문제
  
  RF와 SVM은 CNN 과는 다른 방식으로 작동하며, 이미지에서 추출한 5개의 특징(feature) 값을 사용하여 학습합니다. RF의 최고 성능이 약 0.85이고, SVM은 최고 성능이 약 0.8로 나타난 것으로 보아 CNN 모델에 비해 상대적으로 낮은 성능을 보였습니다. 이러한 낮은 성능의 원인으로는 5개의 feature 수가 하회탈 분류 작업을 하기에는 단순한 특징들이기 때문에, 추가적인 feature 값을 고려해야 한다고 생각합니다.

 
----------
### V. Conclusion

이 연구는 사람들의 얼굴 이미지 데이터셋을 활용하여 얼굴 인식 알고리즘을 기반으로 유사한 하회탈 추정 모델을 개발했습니다. 데이터셋에서 정면이 아닌 사진들은 얼굴의 특징점을 구하여 정면 여부를 판단한 후 걸러냈습니다. 또한, 데이터셋 부족을 보완하기 위해 데이터 증강 기술을 도입하여 14,553개의 이미지 데이터셋을 구축했으며, 다섯 가지 다른 특성값을 추출하여 모델 학습에 사용했습니다.

학습된 SVM, RF, 그리고 CNN 모델들을 평가한 결과, SVM_ECOC 모델은 0.7566의 정확도를, SVM_POLY 모델은 0.80041의 정확도를 보여주었으며, SVM_RBF 모델은 0.80382의 정확도를 기록했습니다. SVM 모델 중에서 SVM_RBF 모델이 가장 높은 정확도를 달성했고, RF 모델은 SVM 모델들보다 더 높은 0.85542의 정확도를 보여주었습니다. 또한, CNN 모델은 다른 모델들보다 우수한 성능을 보여주어 높은 수준인 0.9292의 정확도를 보여주었습니다.

이 연구의 개선 방향으로는 다음과 같은 사항들을 고려할 수 있습니다:
+ 데이터셋 확장: 더 많은 얼굴 이미지 데이터를 수집하고 데이터셋을 더 다양화하여 모델의 학습 데이터를 늘립니다. 이는 모델의 일반화 능력을 향상시키고 성능을 개선하는데 도움이 될 수 있습니다.
+ CNN 모델 개선: CNN 모델의 성능을 높이기 위해 합성곱층 조합을 더 다양하게 시도하고, 최적의 구조를 찾는 노력을 기울입니다. 또한, 사전 학습된 모델을 활용하여 성능을 개선하는 방법을 고려할 수 있습니다.
+ 추가적인 특징 추가: 더 다양한 특성값을 모델에 추가하여 모델이 하회탈의 특징을 더 정확하게 반영하도록 합니다. 특히 이마와 얼굴 면적과 같은 중요한 특징들을 고려하여 성능을 향상시킬 수 있습니다.

마지막으로, 이 연구의 활용 방향에 대해 언급하고자 합니다. 유사한 하회탈을 추정하는 모델은 인공지능을 우리의 문화와도 접목할 수 있다는 사실을 널리 알리는 계기를 만들 수 있을 것입니다. 이 연구 결과는 인공지능 기술이 문화 분야에도 적용될 수 있다는 새로운 가능성을 제시하며, 전통 문화와 현대 기술의 조화를 보여줄 수 있습니다.

또한, 이 모델을 활용하여 '닮은 하회탈을 알아봄으로써 하회탈 문화를 체험하는 것'과 같이 새로운 형태의 문화 체험 프로그램을 제공할 수 있습니다. 사람들은 자신과 닮은 하회탈을 찾아봄으로써 전통 문화에 대한 이해와 관심을 높일 수 있습니다.
