...
Dlib의 68 Landmark 알고리즘을 사용하여 얼굴 이미지 당 68개의 특징점을 얻었습니다. 이 68개의 랜드마크 지점은 얼굴, 눈, 코, 그리고 입 주변의 테두리에 위치합니다. 우리는 이 68개의 특징점 위치를 기반으로 4가지 특징을 만들었습니다. 각 특징은 눈 사이 각도(이마쪽 사잇각 기준), 왼쪽/오른쪽 눈 가로/세로 비율, 코 가로/세로 비율, 턱 각도(얼굴형)을 나타냅니다. 
