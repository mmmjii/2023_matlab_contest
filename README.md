...
CNN 모델의 성능 향상은 검증 세트의 정확도와 학습 세트의 정확도가 함께 상승하면서 나타났습니다. 이는 모델이 훈련 데이터에 과적합되지 않고 일반화되는 경향을 보여주었기 때문입니다. 또한, 두 손실 함수가 비슷한 양상으로 감소하고 있어, 모델이 데이터를 효과적으로 학습하고 있음을 나타냈습니다.

따라서, 이러한 결과들을 토대로 CNN 모델이 문제를 잘 해결하며, 데이터에 대해 잘 일반화되는 좋은 성능을 보여준다고 결론 지을 수 있습니다. 하지만 더 많은 데이터나 다양한 하이퍼파라미터 조정을 통해 더욱 더 성능을 향상시킬 수 있는 가능성도 열어두는 것이 중요하다고 생각합니다.
 
----------
### V. Conclusion

이 프로젝트는 사람들의 얼굴 이미지 데이터셋을 활용하여 얼굴 인식 알고리즘을 기반으로 유사한 하회탈 추정 모델을 개발했습니다. 데이터셋에서 정면이 아닌 사진들은 얼굴의 특징점을 구하여 정면 여부를 판단한 후 걸러냈습니다. 또한, 데이터셋 부족을 보완하기 위해 데이터 증강 기술을 도입하여 14,553개의 이미지 데이터셋을 구축했으며, 다섯 가지 다른 특성값을 추출하여 모델 학습에 사용했습니다.
